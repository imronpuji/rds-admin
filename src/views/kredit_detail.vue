...
<!--     <div v-for="data in cashin">
    ...
    <el-table :key="tableKey" v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%;" @sort-change="sortChange">
        ...
        <el-table-column label="Pembayaran" width="150px" align="center">
            <template slot-scope="{row}">
                <span>{{handleCurrency(row.total)}}</span>
            </template>
        </el-table-column>
        <el-table-column label="Tanggal Bayar" width="150px" align="center">
            <template slot-scope="{row}">
    ...
    methods: {
        getList() {
            ...
            axios.get(`/stock/transaction/detail/${this.$route.params.id}`).then(response => {
                ...
                this.list = response.data.stocktransaction[0].credit
                this.staff = response.data.stocktransaction[0].staff
                this.total = response.data.stocktransaction[0].credit.length
